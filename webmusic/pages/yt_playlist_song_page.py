...
@rx.page(
    route=f"{Route.YOUTUBE_PLAYLIST.value}/[id_playlist]/[id_song]",
    title='WebMusic', 
    on_load=[StateComponents.change_card_info(False), 
             ManageYoutubeApi.verify_redirect_yt_api(
                 "Primero necesita buscar la playlist",
                 Route.YOUTUBE_PLAYLIST.value),
             ManageYoutubeApi.getYoutubeInfo,
             ManageYoutubeApi.getYoutubeMp3]
)
def youtube_playlist_song_page() -> rx.Component:
    return rx.fragment(
